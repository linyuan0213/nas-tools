...
- 站点测试：公开站点补充登录态/登录跳转检测；纯公开站（如 Mikan）不再误报 Cookie 失效
- 自动签到：签到成功/已签判定兼容繁体中文（DiscFan 等繁体站不再误判失败）
- 自动签到：同一天多次调度共享“已签名单”，避免某轮无成功项清空记录导致下一轮全站重复签到
- 自动签到：U2/幼儿园仅 9:00 后开放签到，9 点前调度静默跳过并等待后续补签，不再误报失败

## v4.19.3 (2026-09-08)

### 修复

- 自定义识别词（单条词）备注/帮助为空时 PostgreSQL `NOTE/HELP NOT NULL` 报错，入库前兜底为空串

## v4.19.2 (2026-09-08)

### 修复

- 自定义识别词组：TMDB 自动建组时备注为空导致 PostgreSQL `NOTE NOT NULL` 报错，入库前兜底为空串

## v4.19.1 (2026-09-08)

### 修复

- 备份恢复兼容新旧版本差异：导入前按目标表列过滤，新版本备份恢复到旧版本不再因多余列丢数据
- 站点详情页抓取空/失败时按站退避重试（0.8~6s 递增，恢复正常自动恢复并发），减少触发站点风控；非 2xx 响应记录状态码便于定位

### 优化

- 容器内嵌 nginx 上游端口随 `NEXUS_PORT` 每次启动自动渲染，修改端口后无需改 nginx 配置

## v4.19.0 (2026-09-08)

### 功能

- Web Push 浏览器/移动端系统推送：VAPID 联系邮箱可配置，推送订阅记录日志；前端 PWA（Service Worker + 图标清单）同步配套
- 下载器实时速率与负载：新增 `/download/statistics`，首页展示下载/上传实时速率与限速占用（支持多下载器聚合，无限速时仅显示速率）
- 订阅记录新增 `add_date` 入库时间：首页订阅数“今日新增”统计有可靠口径

### 重构

- 站点解析健康统计收口到仓储层（实体/接口/适配器拆分），逻辑与存储解耦
...
- 刷流误删免费种子：删种/停种/免费自动恢复路径改用详情页 URL 解析免费状态（M-Team 等站 RSS enclosure 为一次性签名链接无法取到 TID）；详情属性解析失败统一收敛为"属性未知"跳过本轮——站点未匹配、TID 提取失败、非 JSON 响应、业务失败响应（如限流 `{"code":1}` 无 data）均不再误判"非免费"触发删种
- 详情页 URL 按站点主域拼接：修复 RSS 域名与主站域名分离的站点（M-Team rss.m-team.cc / kp.m-team.cc）解析不到属性的问题；兼容域名自带 scheme 的站点（观众），RSS 与主站同域站点行为不变
- 观众站点改版后做种数解析失效（新版 `span#seeders`，旧 `div#peercount` 已移除），补充新版选择器，PEER_COUNT 恢复正常
- 删种/停种规则日志补 [Brush] 前缀，日志页可按来源归类检索

## v4.17.0 (2026-09-07)

### 功能
- 首页看板重设计（前端仓库同步发版）：KPI 卡整卡可点击跳转对应页面、环比趋势徽标（电视剧按"今日新增剧数"口径，后端入库统计新增 `tv_series_nums` 按天去重 TMDBID）、系统状态卡（CPU/内存环形图 + 存储进度条 + 下载器负载，30s 轮询）、入库趋势 7/30/90 天切换、最近入库通栏海报墙；配色统一 Tabler 主题变量
- 站点数据统计页重设计（前端仓库同步发版）：站点多时图表 TopN 截断展示并标注、跨图统一站点颜色映射（图例/曲线/扇区同色）、暗色模式图例与坐标轴文字修复、今日流量卡与图表卡样式统一、近 7 天/30 天零增量站点自动过滤
- 系统状态接口新增 CPU/内存占用：`/system/status` 与 `/system/info` 返回 `cpu_percent`、`memory_percent`、`memory_used_mb`、`memory_total_mb`
- M-Team API 域名迁移至 api.m-team.cc：站点配置、字幕下载接口统一按站点定义动态解析 `api.base_url`，移除废弃 `MT_URL` 常量
- M-Team 自动签到适配：浏览器指纹类请求头从站点维护 headers 合并（核心认证头不可覆盖）、UA 缺省回退系统 UA、webversion 缺省取站点当前前端版本、签到成功判定兼容字符串状态码
- CookieCloud 同步补写 LocalStorage：M-Team 等 JWT 登录站点 Cookie 同步后免 Cookie 亦可签到，手动/定时同步行为一致

### 修复
- M-Team 字幕下载与签到在旧域名 api.m-team.io 下的失效问题（随域名迁移一并解决）
