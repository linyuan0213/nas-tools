...
入口：**系统设置 → 目录同步**（`/system/sync`）。监控下载目录，自动识别、重命名并转移文件到媒体库。

![目录同步](assets/images/system/sync.png){ .screenshot }
...
- **未识别目录**：`未识别目录`为无法识别时转移的目录，未识别目录下产生的文件程序**不会主动清理**，建议不配置，未识别记录可在 *文件管理*（`/rename/mediafile`） 功能下处理。
- **同步方式**：参考 [转移方式](#转移方式详解)。
...
- 已安装插件中的`定时目录同步`，使用定时执行全量同步命令的方式同步目录（不推荐）。
