...
def search_medias_for_web(
    ...
    # per-session 进度：WEB 搜索进度按会话隔离，避免并发搜索互相覆盖
    progress_key: str = f"search:{session_id or ''}"
    _process.start(progress_key)
    _process.update_max(ptype=progress_key, value=1, text="开始搜索 ...")
    ...
    try:
        ...
        # 传入 per-session progress key，让索引器站点级进度也写入本会话
        filter_args["progress_key"] = progress_key

        # WEB 搜索未显式指定站点时，应用默认订阅设置的 search_sites
        # （默认设置为空 → site=[] → 不搜索任何站点）
        if "site" not in filter_args and system_config:
            try:
                # 优先用识别出的媒体类型判断默认站点（缺失 media_type 时兜底）
                _default_type = getattr(media_info, "type", None) or mtype
                default_setting = system_config.get(
                    SystemConfigKey.DefaultSubscribeSettingTV
                    if _default_type in (MediaType.TV, MediaType.ANIME)
        ...
        media_list = []
        if search_name_list:
            _process.update_max(ptype=progress_key, value=5, text=f"准备搜索 {content} ...")
            with _web_search_executor(max_workers) as executor:
                all_task = []
                for search_name in search_name_list:
                    task = executor.submit(
                        _searcher.search_medias, search_name, filter_args, media_info, SearchType.WEB
                    )
        ...
        _process.update_max(ptype=progress_key, value=70, text="去重排序中...")
        unique_media_list = []
        media_seen = set()
        for d in media_list:
        ...
        if len(media_list) == 0:
            _process.end(progress_key)
            log.info(f"[Web]{content} 未搜索到任何资源")
            return 1, f"{content} 未搜索到任何资源"
        else:
            log.info(f"[Web]共搜索到 {len(media_list)} 个有效资源")
            _process.update_max(ptype=progress_key, value=90, text=f"共 {len(media_list)} 个有效资源，入库中...")
            media_list = sorted(
                media_list,
                key=lambda x: "{}{}{}".format(
            ...
            _process.update_max(ptype=progress_key, value=100, text=f"搜索完成，共 {len(media_list)} 个资源")
            _process.end(progress_key)

    finally:
        # 保证 per-session key 在成功/失败/无结果等所有路径都结束，SSE 能读到完成态
        _process.end(progress_key)
    return 0, ""
