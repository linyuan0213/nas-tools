...
def init_rbac_system(
    ...
    try:
        ...
        log.info("[RBAC]RBAC系统初始化完成")
        return True
    except Exception as e:
        log.error(f"[RBAC]初始化失败: {e!s}")
        return False
    ...
    try:
        ...
        if existing:
            ...
            if old_hash.startswith(("pbkdf2:", "scrypt:")) or not old_hash:
                ...
                log.info(f"[RBAC]管理员用户 {admin_username} 密码已从旧格式迁移到 Argon2")
            else:
                log.info(f"[RBAC]管理员用户 {admin_username} 已存在")
            return True

        # 系统中已有其他用户（管理员改名/删除过）→ 不再复活默认账号，避免安全隐患
        if user_repo.get_all_users():
            log.info(f"[RBAC]系统已存在用户，跳过创建默认管理员 {admin_username}")
            return True
        ...
        log.info(f"[RBAC]创建管理员用户: {admin_username}")
        return True
    except Exception as e:
        log.error(f"[RBAC]创建管理员用户失败: {e!s}")
        return False
