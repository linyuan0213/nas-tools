...
class MsgWebhookPlugin:
    ...
    def on_enable(self):
        register(Webhook)
        ensure_channel_record(self._message, "webhook", "Webhook")
        self._message.reload_by_type("webhook")
        self.ctx.info("消息渠道插件已启用")

    def on_disable(self):
        disable_channel_record(self._message, "webhook")
        unregister("webhook")
